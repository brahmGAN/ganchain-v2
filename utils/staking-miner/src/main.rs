...
//! # gpu Staking Miner.
//!
//! Simple bot capable of monitoring a gpu (and cousins) chain and submitting solutions to the
...
pub(crate) static mut RUNTIME: AnyRuntime = AnyRuntime::gpu;
...
fn signed_ext_builder_gpu(
	nonce: Index,
	tip: Balance,
	era: sp_runtime::generic::Era,
) -> gpu_runtime_exports::SignedExtra {
	use gpu_runtime_exports::Runtime;
	(
...
macro_rules! any_runtime {
	($($code:tt)*) => {
		unsafe {
			match $crate::RUNTIME {
				$crate::AnyRuntime::gpu => {
					#[allow(unused)]
					use $crate::gpu_runtime_exports::*;
					$($code)*
				},
			}
		}
...
mod tests {
	...
	fn any_runtime_works() {
		...
		let gpu_version = any_runtime! { get_version::<Runtime>() };

		assert_eq!(gpu_version.spec_name, "gpu".into());
	}
}
