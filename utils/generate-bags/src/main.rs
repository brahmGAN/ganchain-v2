...
use gpu_runtime::Runtime as gpuRuntime;
...
impl Runtime {
	fn generate_thresholds_fn(
		...
		match self {
			Runtime::gpu => Box::new(generate_thresholds::<gpuRuntime>),
		}
	}
}
...
struct Opt {
	...
	#[arg(long, ignore_case = true, value_enum, default_value_t = Runtime::gpu)]
	runtime: Runtime,
